...
import { X, Camera, ClipboardCheck } from 'lucide-react-native';
...
interface ReportPunchlistItemModalProps {
    isVisible: boolean;
    onClose: () => void;
    onSuccess: () => void;
    jobId: string;
...
export default function ReportPunchlistItemModal({
    isVisible, onClose, onSuccess, jobId, floors
}: ReportPunchlistItemModalProps) {
    const { user } = useAuth();
    const [selectedFloor, setSelectedFloor] = useState<Floor | null>(null);
    const [selectedUnit, setSelectedUnit] = useState<Unit | null>(null);
    const [selectedArea, setSelectedArea] = useState<Area | null>(null);
    const [itemType, setItemType] = useState('Incomplete Work');
    ...
    const handleSubmit = async () => {
        if (!description.trim()) {
            Alert.alert('Notes Required', 'Please describe what needs to be corrected.');
            return;
        }
        if (!selectedFloor) {
        ...
        try {
            ...
            console.error('Failed to create punchlist item:', err);
            Alert.alert('Error', 'Failed to submit the punchlist item. Please try again.');
        } finally {
            setSubmitting(false);
        }
    };
            ...
            <View className="flex-1 bg-black/50 justify-end">
                <View className={[
                    ...
                    <View className="px-6 py-4 border-b border-slate-100 flex-row justify-between items-center bg-slate-50/50">
                        <View>
                            <Text className="text-slate-400 text-[10px] font-black uppercase tracking-widest">Punchlist</Text>
                            <Text className="text-xl font-black text-slate-900 tracking-tight">Add Punchlist Item</Text>
                        </View>
                    ...
                    <ScrollView className="flex-1 px-6 pt-5" contentContainerStyle={{ paddingBottom: 120 }}>
                        ...
                        {/* Item Type */}
                        <Text className="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-2">Item Type</Text>
                    ...
                    <View className="p-6 border-t border-slate-100 bg-white">
                        <View className="flex-row gap-3">
                            ...
                            <TouchableOpacity
                                ...
                                disabled={submitting}
                                className={`flex-1 h-14 rounded-2xl items-center justify-center shadow-lg ${submitting ? 'bg-slate-400' : 'bg-amber-500'}`}
                            >
                                {submitting ? (
