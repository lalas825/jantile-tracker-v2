...
interface ReportJobIssueModalProps {
    isVisible: boolean;
    onClose: () => void;
    onSuccess: () => void;
    jobId: string;
...
export default function ReportJobIssueModal({
    isVisible, onClose, onSuccess, jobId, floors
}: ReportJobIssueModalProps) {
    const { user } = useAuth();
    const [selectedFloor, setSelectedFloor] = useState<Floor | null>(null);
    const [selectedUnit, setSelectedUnit] = useState<Unit | null>(null);
    const [selectedArea, setSelectedArea] = useState<Area | null>(null);
    const [issueType, setIssueType] = useState('Material Shortage');
    ...
    useEffect(() => {
        if (!isVisible) {
            ...
            setIssueType('Material Shortage');
            setPriority('Medium');
            setDescription('');
            setPhotoUri(null);
        }
    ...
    const handleSubmit = async () => {
        if (!description.trim()) {
            Alert.alert('Notes Required', 'Please describe the issue before submitting.');
            return;
        }
        if (!selectedFloor) {
            Alert.alert('Location Required', 'Please select at least a floor for this issue.');
            return;
        }
        setSubmitting(true);
        try {
            ...
            console.error('Failed to create job issue:', err);
            ...
            <View className="flex-1 bg-black/50 justify-end">
                <View className={[
                    ...
                    <View className="px-6 py-4 border-b border-slate-100 flex-row justify-between items-center bg-slate-50/50">
                        <View>
                            <Text className="text-slate-400 text-[10px] font-black uppercase tracking-widest">Job Report</Text>
                            <Text className="text-xl font-black text-slate-900 tracking-tight">Report Job Issue</Text>
                        </View>
                    ...
                    <View className="p-6 border-t border-slate-100 bg-white">
                        <View className="flex-row gap-3">
                            ...
                            <TouchableOpacity
                                ...
                                disabled={submitting}
                                className={`flex-1 h-14 rounded-2xl items-center justify-center shadow-lg ${submitting ? 'bg-slate-400' : 'bg-blue-600'}`}
                            >
                                {submitting ? (
                                    ...
                                    <View className="flex-row items-center gap-2">
                                        ...
                                        <Text className="text-white font-black uppercase text-sm tracking-widest">Submit Issue</Text>
                                    </View>
                                )}
                            </TouchableOpacity>
                        </View>
